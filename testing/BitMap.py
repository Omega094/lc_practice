...
def setbit_lower(A, x, n):
    if x >= n:
        return
    #2*x+1 is the left child of current node
    if 2*x+1 <= n and A[2*x+1]==0:
        A[2*x+1] = 1
        setbit_lower(A, 2*x+1, n)
    #2*x+2 is the right child of current node
    if 2*x+2 <= n and A[2*x+2]==0:
        A[2*x+2] = 1
        setbit_lower(A, 2*x+2, n)
...
def set_bit(A, pos, length):
    if not A or pos<0 or length <= 0:
        return
    n = len(A) - 1
    #if 2*pos+1 is smaller, not necessary to iterater till pos + length
    #since it is already coverred. 
    for x in range (pos, min(n+1, min(pos+length, 2*pos+1))):
        ...
        #set ancestors
        while x > 0:
            #Make sure it's sibling is 1, otherwise cannot set ancestor
            #Check if x is right child or left child
...
def clear_bit(A, pos, length):
    if not A or pos<0 or length <= 0:
        return 
    n = len(A)-1
    #Settng 0 is different than setting 1
